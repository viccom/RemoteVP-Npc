...
function onConnected(reconnect=false, uri) {
  ...
  //   logMessage("INFO", "Client Has now connected: [Reconnected: ", reconnect, ", URI: ", uri, "]");
    ...
    mqtt_client.subscribe(['v1/common/api/RESULT','v1/vserial/api/RESULT','VSERIAL/STATUS/#', 'VSERIAL/NOTIFY/#', 'VSERIAL/INFO/#']);
...
function onMessageArrived(message) {
    ...
    if(arr_topic[1]==="STATUS") {
        ...
        if(!$.isEmptyObject(data_message)){
            ...
            vserialStatusOBJ = data_message;
            if(data_message.hasOwnProperty('vserial_is_running')){
                vserial_is_running = data_message.vserial_is_running
            }
        }
        if(gatevserial_ready){
            if(vserial_is_running){
                ...
                $('span.stop_vserial').removeClass('mdui-color-grey');
                $('span.stop_vserial').addClass('mdui-color-red');
            }else{
                $('span.start_vserial').removeClass('mdui-color-grey');
                $('span.start_vserial').addClass('mdui-color-green');
                $('span.stop_vserial').addClass('mdui-color-grey');
                $('span.stop_vserial').removeClass('mdui-color-red');
            }
        }
    ...
    if(arr_topic[3]==="RESULT"){
        ...
        //-------------------------------检测选中网关的Vserial环境 返回---------------------------------
        if(apiResult_message['id'].indexOf("vserial/ready") != -1 ){
            ...
            if(apiResult_message.result) {
                if(apiResult_message.data.ready){
                    ...
                    if ($.isEmptyObject(appinfo)) {
                        content = "网关中Vserial应用未安装";
                        action = "install";
                    } else {
                        if (appinfo.name !== "APP00000378") {
                            content = "网关中Vserial应用需要升级";
                            action = "upgrade";
                        } else {
                            content = "网关中Vserial应用未启动";
                            action = "start";
                        }
                    }
                            ...
                            buttons: [
                                ...
                                },
                                {
                                    ...
                                    onClick: function (inst) {
                                        mdui.snackbar({
                                            message: '修复网关Vserial环境！'
                                        });
                                        if (mqttc_connected) {
                                            console.log('修复网关Vserial环境');
                                            var gate_sn = $("span.dest_sn").text();
                                            var auth_code = $("input[name='accesskey']").val();
                                            if (gate_sn !== '') {
                                                var message = new Paho.Message(JSON.stringify({
                                                    "id": 'vserial/action/' + clientId + '/' + Date.parse(new Date()).toString(),
                                                    "gate": gate_sn,
                                                    "action": action,
                                                    "auth_code": auth_code
                                                }));
                                                message.destinationName = 'v1/vserial/api/action';
                                                message.qos = 0;
                                                mqtt_client.send(message);
                                            }
        ...
        //----------------------------根据检测反馈修复网关Vserial 返回------------------------------------
        if (apiResult_message['id'].indexOf("vserial/action") != -1) {
            set_api_result(apiResult_message);
            mdui.snackbar({
                message: '修复完毕，再次检测网关Vserial环境！'
            });
            get_gatevserial_ready();
        }
        //-----------------------------启动Vserial 返回-----------------------------------
        if(apiResult_message['id'].indexOf("vserial/start") != -1 ){
            in_start_action = false;
            set_api_result(apiResult_message);
            if(apiResult_message.result) {
                vserial_is_running = true;
            }


        }
        //----------------------------停止Vserial 返回------------------------------------
        if(apiResult_message['id'].indexOf("vserial/stop") != -1 ){
            in_stop_action = false;
            set_api_result(apiResult_message);
            if(apiResult_message.result) {
                vserial_is_running = false;
            }
        ...
        //------------------------------保持Vserial心跳 返回----------------------------------
        if(apiResult_message['id'].indexOf("vserial/ping") != -1 ){
            // set_api_result(apiResult_message);
        }
        //----------------------------------------------------------------
